...
export function generateNodeSummary(doc: jsPDF, nodes: nodeInterface[]) {
	...
	doc.text(
		`Receipts: ${nodes.reduce(
			(total, node) => (total += node.receipts.length),
			0,
		)}`,
		cellX,
		cellY + lineOffset,
	...
	doc.text(
		`Minting Receipts: ${nodes.reduce(
			(total, node) =>
				(total += node.receipts.filter((receipt) => receipt.measuredUptime).length),
			0,
		)}`,
		cellX,
		cellY + lineOffset * 2,
	...
	doc.text(
		`Fixup Receipts: ${nodes.reduce(
			(total, node) =>
				(total += node.receipts.filter((receipt) => receipt.fixupStart).length),
			0,
		)}`,
		cellX,
		cellY + lineOffset * 3,
	...
	doc.text(
		`Uptime: ${(
			...
			(total, node) =>
				(total += Math.floor(moment.duration(node.uptime, 'seconds').asDays())),
			0,
		)} days`,
		cellX,
		cellY + lineOffset * 5,
...
export async function getNodeMintingFixupReceipts(nodeId: string) {
	...
	const res = await axios
		.get(`https://alpha.minting.tfchain.grid.tf/api/v1/node/${nodeId}`)
		.then((res) =>
			res.data.map(
				(rec: {
					hash: any;
...
export async function getPrices(api: {
	query: { tfgridModule: { pricingPolicies: (arg0: number) => any } };
}) {
